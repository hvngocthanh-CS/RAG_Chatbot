...
The model is loaded ONCE for the whole module (expensive) via a
...
from backend.services.embeddings import EmbeddingService, _BGE_QUERY_INSTRUCTION
...
def expected_dim(embedding_service):
    """Actual dimension from the loaded model (not the settings default)."""
...
class TestEmbeddingBasics:
    ...
    async def test_embedding_dimension_matches_settings(self, embedding_service, expected_dim):
        assert expected_dim == settings.EMBEDDING_DIMENSION, (
            f"Model dim ({expected_dim}) != settings.EMBEDDING_DIMENSION ({settings.EMBEDDING_DIMENSION})"
        )
    ...
    async def test_embed_query_is_normalized(self, embedding_service):
        """BGE embeddings should be L2-normalized (unit vector)."""
...
# ---------------------------------------------------------------------------
# Asymmetric encoding tests (query vs passage produce different vectors)
# ---------------------------------------------------------------------------

class TestAsymmetricEncoding:
    """BGE uses different encoding for queries vs passages."""

    @pytest.mark.asyncio
    async def test_query_and_passage_differ(self, embedding_service):
        """Same text, but query-encoded vs passage-encoded should differ."""
        ...
        assert sim < 0.99, (
            "Query and passage embeddings should differ due to BGE instruction prefix"
        )

    @pytest.mark.asyncio
    async def test_relevant_query_passage_high_similarity(self, embedding_service):
        """A relevant query should have high cosine similarity with its passage."""
        ...
        assert sim > 0.5, f"Expected high similarity for relevant pair, got {sim:.3f}"

    @pytest.mark.asyncio
    async def test_irrelevant_query_passage_lower_similarity(self, embedding_service):
        """An irrelevant passage should score lower than a relevant one."""
        query_emb = await embedding_service.embed_query(
            "What is the annual leave entitlement?"
        )

        relevant_emb = (await embedding_service.embed_documents([
            "[Leave Policy]\n\nEmployees receive 15 days of annual leave.",
        ]))[0]

        irrelevant_emb = (await embedding_service.embed_documents([
            "[IT Security]\n\nAll passwords must be at least 12 characters.",
        ]))[0]

        sim_relevant = float(np.dot(query_emb, relevant_emb))
        sim_irrelevant = float(np.dot(query_emb, irrelevant_emb))

        assert sim_relevant > sim_irrelevant, (
            f"Relevant ({sim_relevant:.3f}) should score higher than "
            f"irrelevant ({sim_irrelevant:.3f})"
        )
...
class TestSectionChunkEmbedding:
    """Verify that section-aware chunks embed well for retrieval."""

    @pytest.mark.asyncio
    async def test_heading_prefix_improves_topic_signal(self, embedding_service):
        """
        Chunks WITH heading prefix should be more similar to a topic-matched
        query than the same text WITHOUT the prefix.

        This validates the Section Chunker's design: prepending [Heading]
        gives the embedding model a stronger topic signal.
        """
        query_emb = await embedding_service.embed_query(
        ...
        # Heading prefix should boost similarity (or at least not hurt it)
        assert sim_with >= sim_without - 0.05, (
            f"Heading prefix should help: with={sim_with:.3f} vs without={sim_without:.3f}"
        )

    @pytest.mark.asyncio
    async def test_end_to_end_chunk_then_embed(self, embedding_service, expected_dim):
        """Full pipeline: chunk text blocks -> embed -> verify dimensions."""
        ...
        assert len(chunks) >= 1
        assert chunks[0]["content"].startswith("[Performance Reviews]")
...
class TestBatchProcessing:
    """Verify correct handling of batches larger than batch_size=32."""

    @pytest.mark.asyncio
    async def test_large_batch_correct_count(self, embedding_service):
        """70 texts -> should still produce exactly 70 embeddings."""
        texts = [f"Document number {i} about various topics." for i in range(70)]
        embeddings = await embedding_service.embed_documents(texts)
        assert len(embeddings) == 70

    @pytest.mark.asyncio
    async def test_single_text_batch(self, embedding_service, expected_dim):
        """Edge case: batch of 1."""
        embeddings = await embedding_service.embed_documents(["Single document."])
